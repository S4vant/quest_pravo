...
const LABOUR_CONTRACT_DISTRACTOR_WORDS = [
    "РЕЗУЛЬТАТ",
    "АКТВЫПОЛНЕННЫХРАБОТ",
    "ПОДРЯД",
    "УСЛУГИ",
...
const ALL_WORDS_STAGE_5 = [
    ...LABOUR_CONTRACT_WORDS,
    ...LABOUR_CONTRACT_DISTRACTOR_WORDS
];

// правильные слова для проверки
const TARGET_WORDS_SOURCES = LABOUR_CONTRACT_WORDS;
...
async function saveResult5(isCorrect) {
    try {
        ...
        const res = await fetch("/api/stage/2/q/5", {
            method: "POST",
